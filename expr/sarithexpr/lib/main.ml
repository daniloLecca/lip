...
| False -> BoolT
| Zero -> NatT
| Succ e -> (match typecheck e with NatT -> NatT | _ -> raise (TypeError (string_of_expr (Succ e) ^ " has type Nat, but type Bool was expected")))
| Pred e -> (match typecheck e with NatT -> NatT | _ -> raise (TypeError (string_of_expr (Pred e) ^ " has type Nat, but type Bool was expected")))
| IsZero e -> (match typecheck e with NatT -> BoolT | _ -> raise (TypeError (string_of_expr (IsZero e) ^ " has type Nat, but type Bool was expected")))
| Not e -> (match typecheck e with BoolT -> BoolT | _ -> raise (TypeError (string_of_expr (Not e) ^ " has type Bool, but type Nat was expected")))
| And(e1, e2) -> (match (typecheck e1, typecheck e2) with (BoolT, BoolT) -> BoolT | (NatT, _) -> raise (TypeError (string_of_expr (e1) ^ " has type Bool, but type Nat was expected"))| (_, NatT) -> raise (TypeError (string_of_expr (e2) ^ " has type Bool, but type Nat was expected")))
| Or(e1, e2) -> (match (typecheck e1, typecheck e2) with (BoolT, BoolT) -> BoolT | (NatT, _) -> raise (TypeError (string_of_expr (e1) ^ " has type Bool, but type Nat was expected"))| (_, NatT) -> raise (TypeError (string_of_expr (e2) ^ " has type Bool, but type Nat was expected")))
